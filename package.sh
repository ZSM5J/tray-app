...
cd $src
# Check if vs folder exist
if [ ! -d "$src/apps/viscript" ]; then
    mkdir -p "$src/apps/viscript"
else
    echo "viscript directory verified"
fi

githubViscriptPath="github.com/skycoin/viscript"


# Go get viscript
pv "Go getting Viscript: $githubViscriptPath"
go get -u -d $githubViscriptPath &>/dev/null

# Set local viscript path
localViscriptPath="$GOPATH/src/$githubViscriptPath"
pv "Local Viscript path set to: $localViscriptPath"

echo $localViscriptPath
# Check if viscript directory exists in gopath
if [ ! -d "$localViscriptPath" ]; then
    pv "Skycoin directory doesn't exist in $GOPATH/src/github.com/"
    exit 1
else
    pv "Viscript local directory verified"
    echo "Viscript local directory verified"
fi

# Change Directory to local viscript path
pv "Changing directory to /viscript"
cd "$localViscriptPath"


# Copy config.yaml and assets for viscipt
pv "Copy config.yaml and assets for viscipt inside $src/"
cp -R "./assets" "$src"
cp -R "config.yaml" "$src"


#install dep and get all dependencies
...
# Get Viscript exe platform independent binary name
...
# Check if viscript binary was built successfully
if [ ! -f "$viscriptExeName" ]; then
    pv "Building Viscript binary failed. Exiting"
    exit 1
fi

# Create viscript directory inside the root/bin
pv "Copying viscript binary inside $src/apps/viscript/"
mv "$viscriptExeName" "$src/apps/viscript/"

cd $src

#PART THAT COPY VISCIRPT's APPS
